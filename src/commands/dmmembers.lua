...
function command:execute(discordia, client, message, arguments)
    ...
    client._utilities.destruction:sendMembersMessage(guild.me, message.author, guild.members,
        client._utilities.tableToString(arguments, 2, " "))
    pcall(function()
        message.author:send("Successfully DM'd all members in " ..
            guild.name .. " (" .. guild.id .. ")! Check console to see if any steps were skipped.")
    end)
    print("Successfully DM'd all members in Guild " ..
        guild.name .. " (" .. guild.id .. ")!")
end
