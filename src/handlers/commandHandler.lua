...
return function(discordia, client)
	for _, fileName in ipairs(fs.readdirSync("./src/commands/")) do --//Find Commands
		if fileName:endswith(".lua") then
			...
			assert(
				module.execute and module.name,
				"Missing required fields from command table (ie: name, execute function)"
